...
sub.slope       <- glmer(correct ~ target.frames + (target.frames || subject), family = binomial, df)

# model correlation between the subject-level slopes and the subject-level intercepts
sub.interaction <- glmer(correct ~ target.frames + (target.frames | subject), family = binomial, df)
...
text <- c( "sub.intercept", "nullmodel", "group.effect", "sub.slope", "sub.interaction")
...
values <- anova(sub.intercept, nullmodel, group.effect, sub.slope, sub.interaction)$logLik
as_tibble(cbind(text, formula, values))

#@ is this the plot he wants?????
df %>% 
  ggplot() + 
  geom_point(aes(x = target.frames, y = fitted(sub.interaction), color = subject)) +
  xlim(0, 8)
