...
#@ We can't even run our models. Are they wrong?

# start from the null model
nullmodel <- glm(correct ~ 1, family = binomial, df)

# add subject-level intercepts
sub.intercept <- glmer(correct ~ 1 + (1 | subject), family = binomial, df)

# add a group-level effect of _target.frames_
group.effect <- glmer(correct ~ target.frames + (1 | subject), family = binomial, df)

# add subject-level slopes for _target.frames_
# sub.slope <- glmer(correct ~ target.frames + (subject | target.frames), family = binomial, df)
#@ maxfun < 10 * length(par)^2 is not recommended.

# model correlation between the subject-level slopes and the subject-level intercepts
# sub.interaction <- glmer(correct ~ target.frames * (subject | target.frames), family = binomial, df)
#@ maxfun < 10 * length(par)^2 is not recommended.
...
# text <- c("nullmodel", "sub.intercept", "group.effect", "sub.slope", "sub.interaction")
# logLik_values <- anova(nullmodel, sub.intercept, group.effect, sub.slope, sub.interaction)$logLik
# as.tibble(cbind(text, logLik_values))
...
> \textcolor{blue}{We can't even run our models. Are they wrong?}
...
df %>% 
  ggplot() + 
  geom_point(aes(x = target.frames, y = fitted(m.6.1), color = pas))+
  facet_wrap( ~ pas)
...
RSS <- function(data, par)
{
    ...
    y.hat <- a + ((b-a) / (1+e*((c-x)/(d)))) #@ this feels right? is e <- exp(1)?
...
fit.pas1 <- function(x) optim.sub7.pas1$par[1] + ((optim.sub7.pas1$par[2]-optim.sub7.pas1$par[1]) / (1+exp(1)*((optim.sub7.pas1$par[3]-x)/(optim.sub7.pas1$par[4]))))

fit.pas2 <- function(x) optim.sub7.pas2$par[1] + ((optim.sub7.pas2$par[2]-optim.sub7.pas2$par[1]) / (1+exp(1)*((optim.sub7.pas2$par[3]-x)/(optim.sub7.pas2$par[4]))))

fit.pas3 <- function(x) optim.sub7.pas3$par[1] + ((optim.sub7.pas3$par[2]-optim.sub7.pas3$par[1]) / (1+exp(1)*((optim.sub7.pas3$par[3]-x)/(optim.sub7.pas3$par[4]))))

fit.pas4 <- function(x) optim.sub7.pas4$par[1] + ((optim.sub7.pas4$par[2]-optim.sub7.pas4$par[1]) / (1+exp(1)*((optim.sub7.pas4$par[3]-x)/(optim.sub7.pas4$par[4]))))
