...
```{python}
sorted_indices = np.argsort(abs_eigenvalues) #This array has the indices of the eigenvalues - but sorted so the index with the smallest value is first.
...
plt.figure()
plt.plot(np.log(np.abs(eigenvalues)), 'o', markersize = 2)
plt.show()
```
>97 + 5 = 102
...
abs_cov_W = np.abs(cov_W) # This is just to make it obvious that light is "bad" and dark is "good". Whether the covariance is not interesting right now.
plt.figure()
sns.heatmap(abs_cov_W, square = True)
plt.show()
```
> Looks near perfect! Barely any covariance above 0 is seen. The last couple of points have covariance - maybe this is related to the 5 points that dropped of on the previous graph?
...
# Where do the arguments k and d go????
for i in k:
  regressor.fit(Z, y)
  score = regressor.score(Z, y)
  scores.append(score)
print(scores)
```
