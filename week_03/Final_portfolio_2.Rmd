...
pacman::p_load(tidyverse, readbulk, lme4, dplyr, dfoptim, MuMIn, lmerTest)
```
...
```{r}
#  - partial pooling enables us to access information across all of the participants
...
1) Pick four subjects and plot their Quantile-Quantile (Q-Q) plots for the residuals of their objective response times (_rt.obj_) based on a model where only intercept is modelled  #This is ambigious!!!! Is it a pool in one DF or plots for each individual??? Lav model og plot individualt for hver subject.
...
# Evt lav en facet wrap for at se for de individuelle personer # nej ligemeget
...
```
    ...
    i. comment on these
      ...
      # Den her opg gåk bare ud på at forstå at reaction data ikke er normalfordelt - i stedet for at tjekke reaktionstiderne direkte kigger vi på residuals fra modellen. Det er loigemeget om ens data er normaltfordelt!!! 
      __Det er residualerne der skal være normalfordelte__
      Hvis data er normalfordelt er der større chance for at residualer er normalfordelt
    
    ii. does a log-transformation of the response time data improve the Q-Q-plots? # Lav det på ny for hver subject
    #Sara tip 5 ggarrange(plotnavn1, plotnavn2)
...
2) Now do a partial pooling model modelling objective response times as dependent on _task_? (set __`REML=FALSE`__ in your `lmer`-specification) #bliv ved med kun at bruge experiment, men alle subject
    ** Only difference is how the numbers look, and whether P values show up **
```{r}
model.2.2.1.lmerTest <- lmerTest::lmer(rt.obj ~ task + (1 | subject), data = df_experiment, REML=FALSE)
model.2.2.1.lme4 <- lme4::lmer(rt.obj ~ task + (1 | subject), data = df_experiment, REML=FALSE)


summary(model.2.2.1.lme4)
summary(model.2.2.1.lmerTest)
...
```
    ...
    - singles has the fastest response time and quadruplet is the fastest #Slowest
    Fixed effects:
...
Pair (intercept) is slowest, Singels is fastest.
    
3) Now add _pas_ and its interaction with _task_ to the fixed effects

```{r}
model.2.3 <- lmer(rt.obj ~ task * pas + (1 | subject), df_experiment) #Add REML?
```
...
# Hvis vores model beskriver 0 varians... Er der måske et threshold for hvor lidt varians den vil arbejde med?
```
...
```{r}
# Hvis vi lige laver pas til factor så giver det ikke mening at lave en slope
```
