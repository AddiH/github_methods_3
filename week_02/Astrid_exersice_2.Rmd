...
class(f1df$scenario)
integer_model <- lm(f0mn ~ scenario, data = f1df)
...
class(f1df$scenario)
factor_model <- lm(f0mn ~ scenario, data = f1df)
...
# Factor is more fitting, 1 is not a value but a name for a specific senario. Coding as an integer makes R do dumdum.
...
twoii <- lm(mean_f0mn ~ gender, data = twoi)
twoii
...
qqnorm(residuals(twoii))
qqline(residuals(twoii))
