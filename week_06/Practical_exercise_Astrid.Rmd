...
#import np # methods of numpy can now be accessed as below
# importing packages (similar to library)

print(np.arange(1, 10)) # see the dot
print(numpy.abs(-3))
import numpy as numpy # you can import them with another name than its default
print(numpy.cos(numpy.pi))
from numpy import pi, arange # or you can import specific methods
print(arange(1, 7))
print(pi)

matrix = numpy.ones(shape=(5, 5)) # create a matrix of ones
identity = numpy.identity(5) # create an identity matrix (5x5)
...
1) Do a linear regression based on _x_, _X_ and _y_ below (_y_ as the dependent variable) (Exercise 1.1)  
    1.i. find $\hat{\beta}$ and $\hat{y}$ (@ is matrix multiplication)
```{python}
...
y = y.astype(float)
n_samples = len(y)
y += numpy.random.normal(loc=0, scale=1, size=n_samples)

X = numpy.zeros(shape=(n_samples, 2))
X[:, 0] = x ** 0
X[:, 1] = x ** 1
...
```{python}
# Exercise 1.2
y1 = numpy.array([3, 2, 7, 6, 9])
y2 = numpy.array([10, 4, 2, 1, -3])
y3 = numpy.array([15, -2, 0, 0, 3])
